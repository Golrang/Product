...
import { addNewResultOfSuggestion as addNewResultOfSuggestionService } from "services/result-of-suggestion/resultOfSuggestionSave.service";

import { addFilePrioritization as addFilePrioritizationService } from "services/result-of-suggestion/addFilePrioritization.service";
...
export const useSubmitPrioritization = (stepId: number, id: number) => {
  ...
  const { mutateAsync: addNewResultOfSuggestion } = useMutation(
    addNewResultOfSuggestionService
  );
  ...
  const onSubmit = async (state: TPrioritizationForm) => {
    const newData = {
      ...
      PostponementDate: state.PostponementDate
        ...
        : null,
      Comment_Prioritization: state.Comment,
    ...
    setLoading(true);
    await addNewResultOfSuggestion(newData, {
      onSuccess: (data) => {
